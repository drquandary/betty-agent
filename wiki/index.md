...
- [[open-ondemand-betty]] — Web portal at ood.betty.parcc.upenn.edu; **Jupyter now GA (out of Beta 7/1, PARCC-curated envs only); OOD terminal = login-trouble workaround** (6 sources)
...
- [[slurm-cli-filter]] — ryb's Lua `cli_filter` (defaults `--qos=dgx`); the `--mem`-propagation bug + bashrc rollout; **7/2: central deployment of cli_filter + server-side `job_submit` plugin (AHEAD meeting; prod `slurm.conf` at `/cm/shared/apps/slurm/etc/slurm/`)** (2 sources)
