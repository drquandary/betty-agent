...
- [[glm-5.2]] — z.ai flagship LLM; fast inference via MTP; served for coding on PARCC (Kimi alternative); **migrated to NVFP4 (B200-native 4-bit) serving 6/30 to cut token cost**; fp8 build (prior) lacked vision; **served endpoint hits nginx `413`/~100k context ceiling (7/2), LiteLLM group still `zai-org/GLM-5.2-FP8`** (tentative, 6 sources)
