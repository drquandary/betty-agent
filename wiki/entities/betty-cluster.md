...
- **Dell shipment mix-up (2026-07-01):** **4× PowerEdge R6725** landed at PARCC's Flexential colo where **1× R7725** was expected ([[kenneth-chaney]]). A same-day **Dell call reframed it (~12:03pm):** the 4 boxes are **~99% for Penn Medicine, not PARCC** — a mis-delivery, not a substitution of PARCC's order. PARCC's actual unit is the **R7725** (Brendan "pretty sure we got an R7725"), which was tied to a **seed-program** ([[jaime-combariza]] has the feedback email). Open loose ends: Dave Cohen's status at Flexential (Ken to call), **service tag `8FVY3J4`** for Brendan, and Flexential checking for other packages (one addressed "Attn Jaime C"). *Correction:* the earlier plan to rack the R6725s so PARCC avoids Flexential charges should defer to routing them to Penn Medicine.
  - **R6725 spec (per node, as received):** dual **AMD EPYC 9655** (Zen5 "Turin", 96C/192T each → 192C/384T/node), **1.5 TB DDR5**, dual **100 Gbps Broadcom** NIC, **4× 3.2 TB NVMe SSD**. Dense CPU/NVMe nodes (not DGX GPU boxes). Status: tentative, and now **likely not PARCC's** pending Penn Medicine confirmation.
