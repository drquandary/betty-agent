...
- **Known pitfall — `ModuleNotFoundError: torch` in the custom kernel (open, 2026-07-07).** Jeffrey hit `import torch → ModuleNotFoundError` in his custom kernel; Ryan couldn't reproduce. Two convergent hypotheses: (a) the registered `kernel.json` points at a `sys.executable` that lacks torch (fix: reprint `sys.executable`, compare to the torch env, re-`ipykernel install` if they mismatch); (b) Ryan's leading guess — the notebook is silently attached to a **second, self-made kernel ("PyTorch Zen4 Betty")** whose env **never had torch `pip install`ed at all** (fix: install torch into that env, or re-point the kernel at curated `venv-pytorch-v01`). Ryan's known-good reference: `sys.executable = /vast/parcc/sw/26.1.zen4/envs/venv-pytorch-v01/bin/python`, and he has **no** custom envs in his own "new" dropdown. Note the name drift — the custom "PyTorch Zen4 Betty" is distinct from the curated **"PyTorch 2.10 (Zen4)"** default; that difference is itself the tell it's a separate (possibly empty) env. Jeffrey to diagnose first thing 7/8. This gates the perf deliverable below.
